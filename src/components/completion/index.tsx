...
// import { ToolCallCard, createToolCallProps } from "../ToolCallCard";
import { SearchingIndicator } from "../SearchingIndicator";

// Check if a tool is a Google Workspace tool
const isGoogleWorkspaceTool = (toolName: string): boolean => {
  const name = toolName.toLowerCase();
  return name.includes('gmail') || name.includes('email') || 
         name.includes('calendar') || name.includes('event') ||
         name.includes('drive') || name.includes('file') ||
         name.includes('docs') || name.includes('document') ||
         name.includes('sheets') || name.includes('spreadsheet') ||
         name.includes('slides') || name.includes('presentation') ||
         name.includes('forms') || name.includes('form') ||
         name.includes('tasks') || name.includes('task') ||
         name.includes('search') || name.includes('chat');
};

// Generate URLs for Google Workspace tools - commented out for build
                  ...
                  <div className="mb-4">
                    ...
                    <div className="space-y-2 text-xs">
                      {sortedActivities.map((a: any) => {
                        // Show SearchingIndicator for in-progress Google Workspace activities only
                        if (a.status === "in_progress" && isGoogleWorkspaceTool(a.name)) {
                          return (
                            <div key={a.id} className="mb-3">
                              <SearchingIndicator toolName={a.name} serviceName="" />
                            </div>
                          );
                        }

                        // Show detailed view for completed/error activities
                        ...
                        return (
                          <details
                            key={a.id}
                            className={`tool-activity p-2 rounded border bg-muted/20 overflow-hidden`}
                          >

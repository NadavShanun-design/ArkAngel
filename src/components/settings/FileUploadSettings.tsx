...
export const FileUploadSettings = ({ showHeader = true }: { showHeader?: boolean }) => {
  ...
  const handleToggle = async (fileId: string) => {
    setBusyId(fileId);
    try {
      const updated = await safeInvoke<FileInfo>('toggle_file_context', { fileId });  // Changed to camelCase for Tauri
      if (updated) {
  ...
  const handleDelete = async (fileId: string, name: string) => {
    ...
    setBusyId(fileId);
    try {
      const res = await safeInvoke('delete_uploaded_file', { fileId });  // Changed to camelCase for Tauri
      if (res !== null) {
